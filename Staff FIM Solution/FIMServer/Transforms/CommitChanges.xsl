						...
						<xsl:otherwise>
							...
							<table border="0" class="tableClass2">
								...
								<tbody>
									...
									<xsl:apply-templates select="Results/ImportObject[TargetObjectIdentifier][not(SourceObjectIdentifier)]" mode="summary">
										<!--<xsl:sort select="Changes/ImportChange[AttributeName='DisplayName']/AttributeValue" order="ascending" data-type="text" />-->
										<xsl:sort select="ObjectType" order="ascending" data-type="text" />
										<xsl:sort select="State" order="ascending" data-type="text" />
									</xsl:apply-templates>
		...
		<tr>
			...
			<td class="tdHeader">
				<xsl:choose>
					...
					<xsl:when test="State/text()='Delete'">
						<xsl:value-of select="TargetObjectIdentifier/text()" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="AnchorPairs/JoinPair/AttributeValue" />
					</xsl:otherwise>
			...
			<td class="tdValue">
				<xsl:choose>
					...
					<xsl:when test="State/text()='Delete'">
						<xsl:text disable-output-escaping="yes">Deletion of object by GUID (need to look up in target xml file)</xsl:text>
					</xsl:when>
					<xsl:otherwise>

...
Version: updated 18/2/2013 to include rename template
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns="http://www.w3.org/1999/xhtml"
			...
			<body>
				...
				<DIV class="c1">
                    ...
                    <!--<th>AttrValNew</th>-->
								...
								<tbody>
                  <xsl:apply-templates select="a:mmsml/a:directory-entries/a:delta[@operation='update'][@newdn]" mode="renames">
                  ...
                  <xsl:apply-templates select="a:mmsml/a:directory-entries/a:delta/a:attr/a:value[1]" mode="changes">
                    <xsl:sort select="../../@dn" order="ascending" data-type="text" />
                  </xsl:apply-templates>
                </tbody>
							</table>
  ...
  <xsl:template name="getObjectClass">
	<xsl:param name="objectClass" />
	<xsl:param name="attrName" />
	<xsl:param name="dn" />
	<xsl:choose>
		<xsl:when test="$objectClass">
			<xsl:value-of select="$objectClass" />
		</xsl:when>
		<xsl:when test="$attrName='info'">group</xsl:when>
		<xsl:when test="$attrName='groupType'">group</xsl:when>
		<xsl:when test="$attrName='managedBy'">group</xsl:when>
		<xsl:when test="$attrName='member'">group</xsl:when>
		<xsl:when test="$attrName='msExchHideFromAddressLists'">user</xsl:when>
		<xsl:when test="$attrName='physicalDeliveryOfficeName'">user</xsl:when>
		<xsl:when test="$attrName='userAccountControl'">user</xsl:when>
		<xsl:when test="contains($dn,'Group')">group</xsl:when>
		<xsl:when test="contains($dn,'User')">user</xsl:when>
	</xsl:choose>
  </xsl:template>
  <xsl:template match="a:delta" mode="renames">
    <tr>
      <!-- Group -->
      ...
      <!-- Object Class -->
      <td class="tdValue">
		<xsl:call-template name="getObjectClass">
			<xsl:with-param name="objectClass" select="@rubbish" />
  ...
  <xsl:template match="a:value" mode="changes">
    <tr>
      ...
      <td class="tdValue">
		<xsl:call-template name="getObjectClass">
			<xsl:with-param name="objectClass" select="../../a:primary-objectclass" />
      ...
      <!-- AttrValDel -->
      <td class="tdValue">
        <xsl:value-of select="../a:value[@operation='delete']/text()" />
      </td>
      <!-- AttrValAdd -->
      <td class="tdValue">
		<xsl:choose>
			<xsl:when test="../a:value[@operation='add']">
				<xsl:value-of select="../a:value[@operation='add']/text()" />
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="../a:value[not(@operation)]/text()" />
			</xsl:otherwise>
		</xsl:choose>
      </td>
      <!-- AttrValNew -->

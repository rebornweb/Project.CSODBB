...
#
# The DEEWR FIM development ("pilot") environment includes various policy objects which are not for migration.  In order to exclude these from 
...
#
# Also, this script is a further variation to the standard in order for the DEEWR FIM design to accommodate the dependencies on custom FIM schema synchronized 
...
#
# In part 1 we are only interested in that policy which excludes all dependencies on the ma-data and mv-data objects in the portal, but at the same
# time grants the FIM Synchronization Service account the rights it needs to refresh the FIM Synchronization Server schema.
# 
# Bob Bradley, Dec 2011

if(@(get-pssnapin | where-object {$_.Name -eq "FIMAutomation"} ).count -eq 0) {add-pssnapin FIMAutomation}

$policy_filename = "E:\Scripts\Output\policy_part1.xml"
...
$policy = Export-FIMConfig -customConfig "/Function","/SynchronizationFilter",`
 "/SearchScopeConfiguration[not(starts-with(DisplayName,'AAA'))]","/HomepageConfiguration","/ObjectVisualizationConfiguration",`
 "/NavigationBarConfiguration[not(starts-with(DisplayName,'AAA'))]","/PortalUIConfiguration","/ConstantSpecifier",`
 "/EmailTemplate","/FilterScope",`
 ...
 "/WorkflowDefinition[not(starts-with(DisplayName,'AAA'))]",`
 "/ManagementPolicyRule[GrantRight='true' and not(starts-with(DisplayName,'AAA'))]"  -MessageSize 9999999 -AllLocales
if ($policy -eq $null)
{

# The following section is a straight copy of FIMPowerShell.ps1 from http://technet.microsoft.com/en-us/library/ff720152(v=ws.10).aspx
...
function GetResourceForCreds
{
    PARAM($ObjectIdentifier, $Uri = $DefaultUri, $Credential)
    END
    {
        $object = Export-FIMConfig -Credential $Credential -CustomConfig $([System.String]::Format("/*[ObjectID='{0}']", $ObjectIdentifier)) -Uri $Uri
        if($object -eq $null)
        {
            Write-Host "Object was not found."
        }

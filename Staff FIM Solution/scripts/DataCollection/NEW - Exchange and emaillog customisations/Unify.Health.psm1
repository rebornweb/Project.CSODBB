...
"@
        ...
        if ($Runs)
        {
            ...
            foreach ($Run in $Runs | where {$_.sync_errors_xml.GetType().Name -ne "DBNull"})
            {
        ...
        if ($Requests)
        {
            ...
            foreach ($Request in $Requests)
            {
                ...
                if ($arr.Count -ge 1000)
                {
                    ...
                    $i += 1
                    Write-Log -ErrorLevel Information -Console -Message ("Export-FIMServiceRequests,Saved {0} Requests to file {1}" -f ($i * 1000).ToString(),$FilePath)
                    $arr = @()
                }
            }
            $RequestsJSON = Convert-ArrayToJson -Compress -UnconvertedArray $arr
            Add-ContentToFile -FilePath $FilePath -ContentToAdd $RequestsJSON
            Write-Log -ErrorLevel Information -Console -Message ("Export-FIMServiceRequests,Saved to {0} Requests file {0}" -f ($i * 1000 + $arr.count).ToString(),$FilePath)
        }
...
#>
    ...
    END
    {
        ...
        foreach ($DB in $Parameters.Infra_DB_All)
        {
            ...
            $arr = @()
            if ($DBSize) {$arr += $DBSize}
            if ($DBIndexes) {$arr += $DBIndexes}

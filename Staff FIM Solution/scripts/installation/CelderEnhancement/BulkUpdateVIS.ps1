...
$tempCredential = New-Object System.Management.Automation.PsCredential "dbb\svcadlds", $password
...
#$filter = "(&(objectClass=user)(!csodbbCeIder=*)(employeeID=2812836200865))"
...
foreach ($key in $userinfoHT.keys) {
    ...
    if ($empID.Length -ge 9) {
        ...
        if ($reset) {
            ...
            $ModAttr.Add("$($empID.Substring($startPos))X") | Out-Null
        }

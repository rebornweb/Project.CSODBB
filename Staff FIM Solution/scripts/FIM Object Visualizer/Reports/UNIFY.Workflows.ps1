...
#    or (ObjectID=/FilterScope/AllowedAttributes) or (ObjectID=/FilterScope/AllowedMembershipReferences)]"
 $data = export-fimconfig -uri http://localhost:5725/resourcemanagementservice -customconfig ("/WorkflowDefinition[not(starts-with(DisplayName,'AAA')) and not(starts-with(DisplayName,'QDET')) and not(starts-with(DisplayName,'NAB')) and not(starts-with(DisplayName,'CLINK')) and not(starts-with(DisplayName,'DEEWR'))]",`
    "/ManagementPolicyRule[not(starts-with(DisplayName,'AAA')) and not(starts-with(DisplayName,'QDET')) and not(starts-with(DisplayName,'NAB')) and not(starts-with(DisplayName,'CLINK')) and not(starts-with(DisplayName,'DEEWR')) and ActionWorkflowDefinition=/WorkflowDefinition[not(starts-with(DisplayName,'AAA'))]]",`
    "/EmailTemplate","/FilterScope","/FilterScope/AllowedAttributes","/FilterScope/AllowedMembershipReferences", "/ma-data[not(DisplayName='FIM MA')]",`
    "/SynchronizationRule[not(starts-with(DisplayName,'AAA')) and not(FlowType='0')]") -MessageSize 99999999 -AllLocales

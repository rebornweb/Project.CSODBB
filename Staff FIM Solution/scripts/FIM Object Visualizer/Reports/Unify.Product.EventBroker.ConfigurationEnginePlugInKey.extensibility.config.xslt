...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ms="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:template match="/">
    <xsl:call-template name="EventBrokerConfiguration" />
  </xsl:template>
  <!--======================================================================================-->
  <!-- Template to display the Event Broker Configuration                                   -->
  <!--======================================================================================-->
  <xsl:template name="EventBrokerConfiguration">
    <table id="tdata">
      <xsl:call-template name="tablestyle" />
      <tr>
        <td colspan="5">
          <xsl:call-template name="reportheadstyle" />
          <xsl:text>Event Broker 3.0 - Configuration</xsl:text>
        </td>
      </tr>
      <xsl:for-each select="//ConfigurationEngine">
        <xsl:call-template name="DataItem">
          <xsl:with-param name="property" select="'Backup Location'" />
          <xsl:with-param name="value" select="@backupLocation" />
        </xsl:call-template>
        <xsl:call-template name="DataItem">
          <xsl:with-param name="property" select="'Backup Time'" />
          <xsl:with-param name="value" select="@backupTime" />
        </xsl:call-template>
      </xsl:for-each>
      <tr>

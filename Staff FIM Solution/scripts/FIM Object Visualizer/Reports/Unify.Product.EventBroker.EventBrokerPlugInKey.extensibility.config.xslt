...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ms="urn:schemas-microsoft-com:xslt">
  ...
  <xsl:template match="/">
    <xsl:call-template name="EventBrokerRoles" />
  </xsl:template>
  <!--======================================================================================-->
  <!-- Template to display the Event Broker Roles                                           -->
  <!--======================================================================================-->
  <xsl:template name="EventBrokerRoles">
    <table id="tdata">
      <xsl:call-template name="tablestyle" />
      <tr>
        <td colspan="5">
          <xsl:call-template name="reportheadstyle" />
          <xsl:text>Event Broker 3.0 - Roles</xsl:text>
        </td>
      </tr>
      <xsl:for-each select="//roleAuthorization">
        ...
        <xsl:call-template name="DataItem">
          <xsl:with-param name="property" select="'Role Name'" />
          <xsl:with-param name="value" select="@role" />
        </xsl:call-template>
        <xsl:call-template name="DataItem">
          <xsl:with-param name="property" select="'Anonymous Action'" />
          <xsl:with-param name="value" select="anonymous/@action" />
        </xsl:call-template>

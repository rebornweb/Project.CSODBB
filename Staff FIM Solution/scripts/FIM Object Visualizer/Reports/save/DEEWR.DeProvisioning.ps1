...
#----------------------------------------------------------------------------------------------------------
 ...
 $data = export-fimconfig -uri http://localhost:5725/resourcemanagementservice -customconfig (`
    "/Set[not(starts-with(DisplayName,'AAA')) and (contains(DisplayName,'depart') or contains(DisplayName,'disabled') or contains(DisplayName,'enabled'))]",`
    "/WorkflowDefinition[not(starts-with(DisplayName,'AAA')) and (contains(DisplayName,'depart') or contains(DisplayName,'activate user'))]",`
    "/ManagementPolicyRule[not(starts-with(DisplayName,'AAA')) and (contains(DisplayName,'AD Accounts'))]") -MessageSize 99999999 -AllLocales

...
--			25 Feb 2008, BBradley, modified to use RTrim when trailing spaces found in Prod data
-- =============================================
...
SELECT
	...
	'B' + RTrim(dbo.Employee.EmployeeCode) COLLATE SQL_Latin1_General_CP1_CI_AS As [ID], 
	RTrim(dbo.Employee.EmployeeCode) As [EmployeeNumber], 
...
Select Distinct 
	'B' + RTrim(Employee.EmployeeCode) COLLATE SQL_Latin1_General_CP1_CI_AS As [ID], 
	'group' As [ObjectType], 
...
IF ( 
	...
	UPDATE ([EmployeeStatus]) OR 
	UPDATE ([EmailAccess]) OR 
	UPDATE ([EmploymentTypeID]) OR 
	UPDATE ([DepartmentID]) 
) 
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
	...
	BEGIN
		...
		inner join inserted 
			...
			left join  dbo.EmploymentType
				on EmploymentType.[EmploymentTypeID] = inserted.[EmploymentTypeID] 
	...
	BEGIN
		...
		insert into @upd_table2
			...
			inner join deleted 
				on v.[ContractID] = deleted.[ContractID]
				left join dbo.Department
					on Department.[DepartmentID] = deleted.[DepartmentID] 
				left join  dbo.EmploymentType
					on EmploymentType.[EmploymentTypeID] = deleted.[EmploymentTypeID] 
...
IF ( 
	...
	UPDATE ([Surname]) OR 
	UPDATE ([DOB]) OR 
	UPDATE ([OrganisationID]) OR 
	UPDATE ([Title]) OR 
	UPDATE ([Gender]) OR 
	UPDATE ([ContactNumber]) OR 
	UPDATE ([MobileNumber]) OR 
	UPDATE ([Email]) OR 
	...
	BEGIN
		insert into @upd_table2
		SELECT v.[ObjectType]
			,'B' + RTrim(inserted.EmployeeCode) COLLATE SQL_Latin1_General_CP1_CI_AS As [ID] 
		...
		inner join inserted 
			...
			left join  dbo.Organisation
				on Organisation.[OrganisationID] = inserted.[OrganisationID] 
	...
	BEGIN
		...
		BEGIN
			insert into @upd_table2
			SELECT v.[ObjectType]
				,'B' + RTrim(deleted.EmployeeCode) COLLATE SQL_Latin1_General_CP1_CI_AS As [ID] 
				,deleted.[EmployeeCode] As [EmployeeNumber]
				,deleted.[GivenName]
				,deleted.[Surname]
				,deleted.[Gender] 
				,deleted.[Title] 
				,deleted.[MobileNumber] 
			...
			inner join deleted 
				...
				left join  dbo.Organisation
					on Organisation.[OrganisationID] = deleted.[OrganisationID] 
...
SELECT [ObjectType]
      ,[ID]
      ,[EmployeeNumber]
      ,RTrim([GivenName]) As [GivenName]
      ,RTrim([Surname]) As [Surname]
		,[Gender] 
		,[Title] 
		,RTrim([MobileNumber]) As [MobileNumber] 
		,RTrim([ContactNumber]) As [ContactNumber] 
		,RTrim([Email]) As [Email] 
		,[EmailAccess] 
		,[EmploymentTypeName]
		,[DepartmentName]
      ,[DOB]
      ,RTrim([PreferredName]) As [PreferredName]
      ,[OrganisationName]
      ,[ContractStartDate]
      ,[ContractEndDate]
      ,RTrim([PositionTitle]) As [PositionTitle]
      ,[PositionLocation]
      ,[DaysToContractEnd]
      ,[EmployeeStatus]
      ,[ContractID]
	  ,RTrim([CapsPin]) As [CapsPin]
      ,[OrganisationID]
      ,[EmployeeID]
      ,[ADSCode]
      ,[ADSDescription]
